...
仅清理设备、日志、采集记录、直播评论动作、心跳、命令和版本事件等脏运行数据，保留 `collection_tasks` 任务配置：
