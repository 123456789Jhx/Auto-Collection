...
import { dispatchExistingPublishTaskNow, dispatchPublishConfigNow } from "./publish-scheduler.service";
...
describe("publish scheduler", () => {
  test("does not run the legacy per-device schedule claim path", async () => {
    const source = await Bun.file(new URL("./publish-scheduler.service.ts", import.meta.url)).text();

    expect(source).not.toContain("runPublishDeviceSchedulerTick");
  });
  ...
  test("dispatches one stored external task without pulling additional interface tasks", async () => {
    const [stored] = await db.insert(publishTasks).values({
      configId,
      taskId: "single-" + suffix,
      platform: "DOUYIN",
      accountName: "节点12命中号",
      title: "单条接口直发",
      description: "#丰收 #乡村",
    ...
    const result = await dispatchExistingPublishTaskNow(stored.taskId, "node12-test");
  ...
  test("reports an unmatched claimed task as unpublished", async () => {
    ...
    expect(task.reportStatus).toBe("REPORTED");
  ...
  test("claims three tasks without applying a daily account limit", async () => {
    ...
    expect(result).toMatchObject({ dispatched: 1, reported: 0, busy: 2 });
    const tasks = await db.select().from(publishTasks).where(inArray(publishTasks.taskId, ids));
    expect(tasks).toHaveLength(3);
    expect(tasks.filter((task) => task.status === "DISPATCHED")).toHaveLength(1);
    expect(tasks.filter((task) => task.status === "PUBLISH_BUSY")).toHaveLength(2);
  });

  test("dispatches douyin and queues wechat channels as busy in the same slot", async () => {
    ...
    expect(result).toMatchObject({ dispatched: 1, reported: 0, busy: 1 });
    ...
    expect(douyinTask.status).toBe("DISPATCHED");
    expect(channelsTask.status).toBe("PUBLISH_BUSY");
    ...
    const douyinCommand = commands[0]!;
