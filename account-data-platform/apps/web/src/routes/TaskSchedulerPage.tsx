...
import { createTaskAssignment, getDeviceTaskConfig, getDevices, getTaskAssignments, updateDeviceTaskConfig } from "../lib/api-client";
...
type SchedulerConfigFormValues = {
...
function schedulerConfigFormValues(taskConfig?: TaskConfigRow | null): SchedulerConfigFormValues {
...
function buildLiveCommentBotConfig(taskConfig: TaskConfigRow | undefined, values: SchedulerConfigFormValues) {
  const botConfig = { ...(defaultLiveCommentBotConfig as Record<string, unknown>), ...recordValue(taskConfig?.liveCommentBotConfig) };
  const targetRoom = recordValue(botConfig.targetRoom);
...
function buildP3ExtensionsConfig(taskConfig: TaskConfigRow | undefined, values: SchedulerConfigFormValues) {
  const p3Config = { ...(defaultP3ExtensionsConfig as Record<string, unknown>), ...recordValue(taskConfig?.p3ExtensionsConfig) };
  const defaultCommerce = recordValue((defaultP3ExtensionsConfig as Record<string, unknown>).commerceCardLiveComment);
  const commerceConfig = { ...defaultCommerce, ...recordValue(p3Config.commerceCardLiveComment) };
  const commerceTargetRoom = recordValue(commerceConfig.targetRoom);
...
export function TaskSchedulerPage() {
  ...
  const [configForm] = Form.useForm<SchedulerConfigFormValues>();
  ...
  const configPlatform = configDeviceState?.device.platform || "douyin";
  const configQuery = useQuery({
    queryKey: ["scheduler-device-task-config", configDeviceState?.deviceCode, configPlatform],
    enabled: configOpen && !!configDeviceState?.deviceCode,
    queryFn: () => getDeviceTaskConfig(configDeviceState?.deviceCode || "", configPlatform)
  });
  const configMutation = useMutation({
    mutationFn: async (values: SchedulerConfigFormValues) => {
      if (!configDeviceState?.deviceCode) throw new Error("请先选择要配置的手机");
      const taskConfig = taskConfigFromUnknown(configQuery.data);
      return updateDeviceTaskConfig(configDeviceState.deviceCode, {
    ...
    onSuccess: async () => {
      setConfigOpen(false);
      setNotice({ kind: "success", text: "配置已保存，后台已向手机下发 REFRESH_CONFIG，等待手机领取后生效。" });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ["scheduler-device-task-config"] }),
        queryClient.invalidateQueries({ queryKey: ["devices"] }),
        queryClient.invalidateQueries({ queryKey: ["task-assignments"] })
      ]);
    },
  ...
  useEffect(() => {
    ...
    configForm.setFieldsValue(schedulerConfigFormValues(taskConfigFromUnknown(configQuery.data)));
  }, [configForm, configOpen, configQuery.data]);
  ...
  function openDeviceConfig(row?: DeviceTaskState | null) {
    ...
    configForm.setFieldsValue(schedulerConfigFormValues(null));
    setConfigOpen(true);
  }
  ...
  return (
    <div className="scheduler-page">
      ...
      <Modal
        ...
        title={`配置${configDeviceState ? ` - ${deviceDisplayName(configDeviceState)}` : ""}`}
        open={configOpen}
