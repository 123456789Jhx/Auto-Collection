...
export function TasksPage() {
  ...
  return (
    <Card title="任务配置">
      <Table rowKey={(row) => String((row as any).id)} dataSource={query.data ?? []}>
        ...
        <Table.Column title="视频时长" render={(_, row: any) => `${row.videoMinutesMin}-${row.videoMinutesMax} 分钟`} />
        <Table.Column title="直播时长" render={(_, row: any) => `${row.liveMinutesMin}-${row.liveMinutesMax} 分钟`} />
        <Table.Column title="心跳间隔" render={(_, row: any) => `${row.heartbeatMinutes ?? 1} 分钟`} />
