...
export const deviceAccountBindingSchema = z.object({
  douyinAccountId: z.string(),
  douyinAccountName: z.string(),
  wechatChannelsName: z.string()
}).strict();

export type DeviceAccountBinding = z.infer<typeof deviceAccountBindingSchema>;

const deviceAccountProfileSchema = z.record(z.unknown()).superRefine((value, context) => {
  const bindingKeys = ["douyinAccountId", "douyinAccountName", "wechatChannelsName"];
  if (bindingKeys.some((key) => Object.hasOwn(value, key)) && !deviceAccountBindingSchema.safeParse(value).success) {
    context.addIssue({
      code: "custom",
      message: "账号绑定必须且只能包含三个字符串字段"
    });
  }
});
...
export const updateDeviceSchema = z.object({
  ...
  accountProfile: deviceAccountProfileSchema.nullable().optional()
});
