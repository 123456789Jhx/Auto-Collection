...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\bundle-autojs.ps1 -Version 1.0.0
powershell -ExecutionPolicy Bypass -File .\scripts\publish-autojs-version.ps1
```

该通道只打包 `features/` 与 `domain/`，并生成逐文件 SHA-256 manifest；`config.js`、APK 基座和其他运行文件不会被覆盖。设备校验失败时恢复本地备份，无可用覆盖层时继续使用 APK 内置基线。
...
2. 已开启悬浮窗权限。
3. 截图权限已授权。
4. 网络能访问当前开发机 API：`http://192.168.7.170:3012/api/v1`。
