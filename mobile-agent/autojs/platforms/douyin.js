function createDouyinAdapter(config, logger, ocrEngine, floatyControl, injectedScreenRecognizer, injectedLiveTargetMatcher) {
   ...
   *
   * 业务职责：
   * - 搜索完商城后，抖音可能停在“综合/视频/直播”等列表。
   * - 商品卡养号要求先从“全部”板块下面的商品卡列表点首张卡，所以这里专门找“全部”tab 并点击。
   *
   * 调用位置：
   * - openCommerceCardSearch() 搜索完成后会调用。
   * - tryReuseCommerceCardSearchResult()/recoverCommerceCardSearch() 复用或恢复搜索结果时也会调用。
   */
  function clickCommerceResultTabIfVisible() {
    // waitForElement() 是项目封装的短等待；这里同时查 text 和 desc，是因为不同机型/抖音版本的 tab 文案暴露方式不同。
    var tabNode =
    ...
    // safeClick() 会先尝试节点点击，失败时再用坐标兜底；比直接 click() 更适合抖音这种复杂 View。
   ...
   * 业务职责：
   * - 进入抖音商城。
   * - 搜索后台下发的 searchKeyword。
   * - 搜索完成后切到“全部”板块，给 clickCommerceKeywordCard() 提供商品卡列表页面。
   *
   * 返回值：
   * - true：当前已经处在可浏览的商城搜索结果页。
   * - false：商城入口、搜索框或搜索动作失败，外层 browseCommerceCards() 会结束本轮任务。
   */
  function openCommerceCardSearch(keyword) {
    ...
    // 如果当前屏幕已经是同一个关键词的商城搜索结果，就复用页面，避免重复退出/进入商城。
    if (tryReuseCommerceCardSearchResult(keyword, "before_enter_mall")) {
      return true;
    }
    // enterMall() 负责从抖音当前上下文找到并点击商城入口。
    if (!enterMall()) {
    ...
    // openSearch() 在商城内执行搜索；失败原因会写到 lastSearchFailureReason，外层返回给任务结果。
    if (!openSearch(keyword)) {
    ...
    // 搜索完成后强制点一次“全部”，后续 clickCommerceKeywordCard() 才是在商品卡列表里找首卡。
    clickCommerceResultTabIfVisible();
   ...
   *
   * 业务职责：
   * - 判断一段无障碍文本是否像商品卡，而不是评论、话题、详情参数或普通标题。
   * - 这里用价格、销量、包邮、发货、店铺等信号辅助判断，避免只凭“夏橙”关键词误点当前详情页文本。
   *
   * 调用位置：
   * - isCommerceProductCandidateNode()
   * - findCommerceKeywordProductCardCandidate()
   * - findCommerceRelatedProductCardCandidate()
   * - hasCommerceRelatedProductList()
   */
  function isCommerceProductSignalText(value) {
    return commerceCardDetector.isProductSignalText(value);
  }

  function isCommerceNonProductCardText(value) {
    return commerceCardDetector.isNonProductCardText(value);
  }
   ...
   *
   * 业务职责：
   * - 当“从关键词节点向父级找商品卡容器”的策略没有找到结果时，用当前节点本身做兜底。
   * - 会读取当前节点自己的 text/desc，也会读取少量子节点文本，尽量拼出商品标题、价格、已售等上下文。
   *
   * 注意：
   * - 这个方法只判断“像不像商品卡”，不负责点击。
   * - 真正点击前还会用 bounds 几何范围过滤，避免点到整页容器或不可点击文本。
   */
  function isCommerceProductCandidateNode(node) {
    if (!node) {
      return false;
    }
    // getNodeOwnText() 只拿当前节点 text/desc；collectNodeContextText() 会往下收集子节点文本。
    var ownText = getNodeOwnText(node);
    var contextText = collectNodeContextText(node, 2);
    var combinedText = [ownText, contextText].join("\n");
    // 评论、话题、点赞收藏等也可能在屏幕里出现，先排除这些非商品区域。
    if (isCommerceNonProductCardText(ownText) || isCommerceNonProductCardText(contextText)) {
      return false;
    }
    // 只要当前节点或其子节点上下文包含商品强信号，就允许作为候选。
   ...
   *
   * 业务职责：
   * - 识别商品详情页里的推荐区文案，例如“你可能还会喜欢”“你可能想看”“猜你喜欢”。
   * - browseOpenedCommerceDetail() 会依赖这个判断决定是否开始尝试点击后续推荐商品卡。
   *
   * 当前限制：
   * - 它只能判断屏幕文本里是否出现推荐区信号，不能直接定位推荐区标题节点坐标。
   * - 后续真正修点击问题时，应该进一步找到标题节点 bottom，再只扫描 bottom 下方的卡片区域。
   */
  function isCommerceRelatedZoneText(value) {
    return commerceCardDetector.isRelatedZoneText(value);
  }
   ...
   *
   * 业务职责：
   * - 有些推荐商品卡没有把价格、已售等兄弟节点暴露到同一个无障碍容器里。
   * - 这种情况下只能先用“水果/新鲜/现摘/橙/农家”等标题词做弱判断。
   *
   * 注意：
   * - 这是弱信号，不能单独证明点击成功。
   * - 点击后必须继续用 buildCommerceDetailSignature() 校验页面是否真的变成新商品详情。
   */
  function isCommerceProductTitleLikeText(value) {
    return commerceCardDetector.isProductTitleLikeText(value);
  }
   ...
   *
   * 业务职责：
   * - 判断一个无障碍节点的 bounds 是否像详情页里的推荐商品卡。
   * - 过靠上容易是当前商品标题、购买按钮或详情 tab；过窄可能只是文字行；过高可能是整页容器。
   *
   * 参数说明：
   * - bounds：AutoX.js 无障碍节点的屏幕矩形。
   * - screen：当前屏幕宽高，用比例规则适配不同手机。
   */
  function isCommerceRelatedCardBoundsAllowed(bounds, screen) {
    return commerceCardDetector.isRelatedCardBoundsAllowed(bounds, screen);
  }
   ...
   *
   * 业务职责：
   * - clickCommerceKeywordCard() 用它过滤“全部”列表里的首张商品卡候选。
   * - 搜索结果页商品卡允许出现在更靠上的区域，所以和详情页推荐卡的 bounds 规则分开。
   */
  function isCommerceSearchCardBoundsAllowed(bounds, screen) {
    // 搜索结果页首卡和详情页推荐卡共用部分几何判断，但首卡允许出现在更靠上的搜索结果区域。
   ...
   *
   * 业务职责：
   * - 当同一屏有多个命中关键词的节点时，选最像商品卡的一块去点击。
   * - 分数来源包括父级层级、商品强信号、推荐区状态、节点尺寸和屏幕位置。
   *
   * 注意：
   * - 打分只解决“多个候选选谁”的问题。
   * - 是否真的进入新商品，仍以后续页面签名校验为准。
   */
  function scoreCommerceProductCardCandidate(bounds, screen, combinedText, depth, relatedZoneVisible) {
    return commerceCardDetector.scoreProductCardCandidate(bounds, screen, combinedText, depth, relatedZoneVisible);
  }
   ...
   *
   * 业务职责：
   * - clickCommerceKeywordCard() 先用 textMatches/descMatches 找到包含商品关键词的节点。
   * - 这个方法从该关键词节点开始向父级 parent() 爬，尝试找到更大的商品卡容器。
   *
   * 为什么要向父级爬：
   * - 关键词文本节点本身可能只是标题的一行，点它不一定能进详情。
   * - 父级节点更可能包含图片、标题、价格、已售等整张商品卡区域。
   */
  function findCommerceKeywordProductCardCandidate(keywordNode, keywordRegex, screen) {
    var best = null;
    var bestScore = -1;
    // getNodeOwnText() 只读当前无障碍节点自己的 text/desc，不包含兄弟节点。
    var matchedText = getNodeOwnText(keywordNode);
    // current 从关键词节点开始，逐层 current.parent() 往上找；父节点更可能是完整商品卡容器。
    var current = keywordNode;
    for (var depth = 0; current && depth <= 4; depth++) {
      // node.bounds() 是 AutoX.js/Android 无障碍节点 API，用来拿该节点在屏幕上的坐标。
      ...
      if (bounds && isCommerceSearchCardBoundsAllowed(bounds, screen)) {
        ...
        // collectNodeContextText() 会读取当前节点和少量子节点文本，尽量把标题、价格、已售拼到一起。
        ...
        if (keywordRegex.test(combinedText) &&
          ...
          if (score > bestScore) {
            bestScore = score;
            best = {
              // 保存 node 是为了后面交给 axisClick() 点击；bounds/text 只用于日志和去重。
              node: current,
              score: score,
              bounds: autojsUtils.formatBounds(bounds),
              text: combinedText,
      ...
      try {
        // AutoX.js 的 parent() 可能在节点失效时抛异常，所以这里必须保护，避免整轮任务崩掉。
  ...
  /*
   * 从详情页推荐区关键词节点提升到推荐商品卡容器。
   *
   * 业务职责：
   * - clickCommerceRelatedProductCard() 在详情页推荐区域尝试找下一张商品卡时调用。
   * - 从包含商品关键词的节点向父级找，过滤掉已经点过但没跳转的 skippedBounds。
   *
   * 当前已知问题：
   * - 真机推荐卡标题有时不暴露为无障碍文本，这个方法就找不到真实卡片。
   * - 商品图片、标题、价格可能是兄弟节点，不一定能通过 parent() 聚合到一起。
   * - 所以下一步修复应增加“推荐区坐标 + 卡片几何 + OCR”的扫描方式。
   */
  function findCommerceRelatedProductCardCandidate(keywordNode, keywordRegex, skippedBounds, screen) {
    var best = null;
    var bestScore = -1;
    var matchedText = getNodeOwnText(keywordNode);
    // extractVisibleText() 会读取当前屏幕所有可见无障碍文本；这里只用来判断是否已经进入推荐区。
    var relatedZoneVisible = isCommerceRelatedZoneText(extractVisibleText());
    var current = keywordNode;
    for (var depth = 0; current && depth <= 4; depth++) {
      var bounds = current.bounds && current.bounds();
      // formatBounds() 把 Rect 转成字符串，例如 [55,1384][1025,1514]；用于日志和 skippedBounds 去重。
      ...
      if (bounds && !skippedBounds[boundsKey] && isCommerceRelatedCardBoundsAllowed(bounds, screen)) {
        ...
        if (keywordRegex.test(combinedText) &&
          ...
          if (score > bestScore) {
            bestScore = score;
            best = {
              // 注意：这里现在仍是从关键词节点向父级找，不能覆盖兄弟节点卡片；这是当前需要继续修的点。
              node: current,
              score: score,
              bounds: boundsKey,
              text: combinedText,
      ...
      try {
        // 向父级聚合是为了从“夏橙标题行”找到“整张商品卡”；如果父级文本拿不到价格/图片，就会漏识别。
        current = current.parent && current.parent();
      } catch (error) {
        current = null;
      }
  ...
  /*
   * 点击搜索结果页“全部”板块里的目标商品卡。
   *
   * 业务职责：
   * - 这是商品卡养号的第一跳：从商城搜索结果列表进入第一张商品详情页。
   * - 它只处理搜索结果页首卡，不处理详情页里的推荐商品卡。
   *
   * 调用链路：
   * - browseCommerceCards()
   *   -> clickCommerceKeywordCard()
   *   -> findCommerceKeywordProductCardCandidate()
   *   -> axisClick()
   */
  function clickCommerceKeywordCard(matchKeywords) {
    // buildContainsRegex(["秭归", "夏橙"]) 会生成可给 textMatches()/descMatches() 使用的正则。
    var keywordRegex = buildContainsRegex(matchKeywords);
    if (!keywordRegex) {
      return false;
    }
    var nodes = [];
    // textMatches/descMatches 是 AutoX.js 无障碍选择器：
    // textMatches 查节点 text，descMatches 查 contentDescription；两者都可能承载商品标题。
    pushFoundNodes(nodes, textMatches(keywordRegex));
    pushFoundNodes(nodes, descMatches(keywordRegex));
    // getScreenSize() 返回当前屏幕宽高；后面所有 bounds 判断都按这个宽高做比例过滤。
    var screen = autojsUtils.getScreenSize();
    var best = null;
    var bestScore = -1;
    for (var i = 0; i < nodes.length; i++) {
    ...
    // axisClick() 是项目封装：优先点节点 bounds 中心坐标，并写点击日志；不是浏览器 DOM click。
    autojsUtils.axisClick(best.node, logger);
    // 点击后给抖音页面跳转和动画留时间，随机等待也能避免固定节奏。
    autojsUtils.sleepRandom(1800, 2600);
    return true;
  }
   ...
   * 业务职责：
   * - 这是商品卡养号的第二跳及后续循环入口。
   * - 当前商品详情页往下滑到推荐区后，本方法负责找“推荐商品卡”并点击进入下一张详情页。
   * - 点击后是否真的跳转成功，不在这里判断，而是在 browseOpenedCommerceDetail() 里用页面签名校验。
   *
   * 调用链路：
   * - browseOpenedCommerceDetail()
   *   -> hasCommerceRelatedProductList()
   *   -> clickCommerceRelatedProductCard()
   *   -> findCommerceRelatedProductCardCandidate()
   *   -> axisClick()
   *
   * 当前待修复点：详情页推荐商品卡识别与点击。
   *
   * 真实日志现象：
   * 1. 脚本已经能从商城搜索结果页“全部”板块点击首张夏橙商品卡。
   * 2. 进入商品详情页后也能持续下滑，并且屏幕文本已经出现“你可能还会喜欢”。
   * 3. 日志里出现过“点击详情页后续商品卡片区域”，但随后又记录
   *    “commerce related product click did not open a new product”。
   * 4. 这说明不是网络、后台命令或心跳问题，而是这里选出来的候选节点
   *    不是能进入下一张商品详情的推荐商品卡主点击区域。
   *
   * 当前实现的问题：
   * 1. 这里仍然从 textMatches/descMatches(productKeywords) 出发找节点。
   *    如果推荐商品卡标题没有暴露在无障碍节点里，代码就无法命中真实卡片。
   * 2. 如果当前商品详情页顶部或参数区残留了“夏橙”标题文本，代码可能把当前商品标题、
   *    详情参数、规格、保障说明或不可跳转文本当作“后续商品卡”。
   * 3. findCommerceRelatedProductCardCandidate() 只是从关键词节点向父级聚合，
   *    但真机上商品图片、价格、标题、已售和可点击区域可能是兄弟节点，不一定都在父级文本里。
   * 4. 当前点击点来自候选 bounds 的中心点；如果候选 bounds 只是标题/说明行，
   *    即使文字看起来正确，点击也可能不会打开新商品。
   *
   * 推荐修复顺序：
   * 1. 先定位推荐区标题节点，例如“你可能还会喜欢”“你可能想看”“猜你喜欢”“推荐商品”。
   *    记录推荐区标题的 bottom，后续商品卡候选必须在这个 y 坐标下方。
   * 2. 只扫描推荐区标题下方的商品卡区域，排除当前商品主详情区域：
   *    - 进店、客服、加购物车、领券购买、立即购买、专享价。
   *    - 商品/评价/详情/推荐 tab。
   *    - 保障、规格、净重、产地、售卖方式、品质认证、是否临期等参数区。
   * 3. 在推荐区下方按“卡片几何”找候选，而不是只按关键词节点找：
   *    - 左右两列或单列卡片宽度合理。
   *    - 有图片区域、价格、已售、新人价、包邮、发货等商品信号。
   *    - bounds 中心点位于推荐区下方可点击区域。
   * 4. 如果无障碍文本没有读到 productKeywords，但推荐卡区域肉眼可见标题，
   *    应截取推荐区下方卡片区域走 OCR，再用 OCR 文本匹配“夏橙/秭归”等商品词。
   * 5. 命中后点击整张推荐商品卡的图片或卡片中心，不要点击标题文字节点本身。
   * 6. 点击后继续保留页面签名校验：
   *    - afterRelatedSignature 必须不同于 beforeRelatedSignature。
   *    - 新页面仍要命中商品关键词。
   *    - 未跳转时把 bounds 写入 skippedRelatedBounds，继续下滑找下一张。
   *
   * 简单说：这里下一版不要再从“夏橙文本节点”倒推商品卡，
   * 而应该从“推荐区位置 + 商品卡几何 + OCR/无障碍文本”共同确定卡片，再点卡片主体。
   */
  function clickCommerceRelatedProductCard(matchKeywords, skippedBounds) {
    ...
    // skippedBounds 记录“刚点过但没有打开新商品”的区域，避免同一块标题/参数区域被反复点击。
    skippedBounds = skippedBounds || {};
    var nodes = [];
    // 目前这里的入口仍是无障碍文本：先找包含商品词的 text/desc 节点，再尝试提升到商品卡容器。
    // 真机问题就在这里：推荐商品卡标题可能不在 text/desc 里，后续应补 OCR/几何扫描入口。
    pushFoundNodes(nodes, textMatches(keywordRegex));
    pushFoundNodes(nodes, descMatches(keywordRegex));
    var screen = autojsUtils.getScreenSize();
    var best = null;
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      // node.text()/node.desc() 是 Android 无障碍节点字段；不同抖音版本可能只填其中一个。
      var value = String((node.text && node.text()) || (node.desc && node.desc()) || "");
      if (/进店逛逛|客服|加购物车|领券购买|立即购买|专享价/.test(value)) {
        continue;
      }
      // 主策略：从命中的“夏橙”节点往父级找商品卡容器。
      ...
      if (candidate && candidate.score > bestScore) {
        // 多个候选同时命中时，选分数最高的一个点击；日志里 best.bounds 就是最终点击区域。
        bestScore = candidate.score;
        best = candidate;
      }
    }
    ...
    // 当前点击的是候选节点中心点；如果候选只是文本行，点击后可能不会跳转，这就是日志里未打开新商品的原因之一。
    autojsUtils.axisClick(best.node, logger);
    autojsUtils.sleepRandom(1800, 2600);
    return {
      clicked: true,
  ...
  /*
   * 判断详情页是否已经出现后续商品卡/推荐区信号。
   *
   * 业务职责：
   * - browseOpenedCommerceDetail() 每轮读取当前屏幕文本后，会先调这个方法。
   * - 返回 true 才会尝试 clickCommerceRelatedProductCard()，否则继续停留或下滑。
   *
   * 判断顺序：
   * - 当前屏幕必须仍然命中目标商品关键词。
   * - 优先看后台配置的 recommendationSignals。
   * - 再看代码内置的推荐区标题。
   * - 最后用价格、已售、包邮、发货等商品卡信号兜底。
   */
  function hasCommerceRelatedProductList(detailText, matchKeywords, targetRoom, recommendationSignals) {
    return commerceCardDetector.isRelatedProductListText(
      detailText,
   ...
   * 业务职责：
   * - 点击推荐商品卡前后各取一次屏幕文本。
   * - 如果签名没有变化，说明刚才点击大概率没打开新商品详情。
   * - 如果签名变化，还要继续校验新页面是否命中目标商品关键词和商城详情上下文。
   *
   * 为什么要去数字：
   * - 价格、销量、倒计时、库存等数字可能变化。
   * - 这些变化不代表页面真的跳到了下一张商品卡。
   */
  function buildCommerceDetailSignature(textValue) {
    return commerceCardSignature.buildDetailSignature(textValue);
  }
   ...
   *
   * 业务目标：
   * - 先从商城搜索结果页“全部”板块点击目标商品卡。
   * - 进入商品详情页后往下滑，找到详情页里的推荐商品卡。
   * - 推荐商品卡命中目标关键词后继续点进下一张详情页。
   * - 在 scanMinutes 时间内重复这个链路。
   *
   * 主调用关系：
   * - runV2ProductNurture() 在 runner 层调用本方法。
   * - 本方法负责 UI 侧完整浏览流程。
   * - 打开首卡后交给内部 browseOpenedCommerceDetail() 处理详情页循环。
   *
   * 返回值：
   * - success=true：至少浏览过一张商品卡。
   * - success=false：搜索失败、上下文恢复失败、手动暂停或一直找不到商品卡。
   */
  function browseCommerceCards(options) {
    options = options || {};
    var startedAt = Date.now();
    // searchKeyword 是商城搜索框里输入的词；matchKeywords 是用来判断卡片是否符合目标商品的词。
    var searchKeyword = String(options.searchKeyword || "").replace(/\s+/g, " ").trim();
    var matchKeywords = options.matchKeywords || [];
    var liveSignals = options.liveSignals || [];
    ...
    // 每个等待/循环点都要检查后台控制命令，保证暂停任务能及时生效。
    function isInterrupted(source) {
    ...
    // 分片 sleep：长等待拆成 1 秒以内的小片，每片之间检查暂停/停止命令。
     ...
     * - 当前已经进入某一张商品详情页。
     * - 先停留一段时间，期间可选处理直播入口。
     * - 滑到推荐区后，尝试点击命中关键词的后续推荐商品卡。
     * - 点击成功后不返回搜索结果页，而是在新商品详情页继续重复。
     *
     * 关键保护：
     * - hardEndAt：整个商品卡阶段的硬截止时间，避免无限循环。
     * - detailEndAt：当前详情页的计划停留截止时间，没找到推荐卡会分段延长。
     * - skippedRelatedBounds：记录点过但没跳转的区域，避免重复点同一个错误位置。
     * - buildCommerceDetailSignature()：点击前后对比页面签名，确认是否真的进入新商品。
     */
    function browseOpenedCommerceDetail(attempt, hardEndAt, remainingCards, stayUntilHardEnd) {
      var detailStartedAt = Date.now();
      // detailEndAt 是当前商品详情页计划停留到的时间；如果没找到推荐卡，会按 15 秒一段向后延长。
      var detailEndAt = Math.min(hardEndAt, detailStartedAt + dwellSeconds * 1000);
      // 打开首个详情页本身就算浏览 1 张卡；后续每成功跳到一个新详情页再加 1。
      var detailBrowsedCount = 1;
      var openedLive = false;
      // 进入详情页后先看一段时间，15 秒后才开始下滑，避免刚进页就错过顶部信息/直播入口。
      var nextSwipeAt = detailStartedAt + 15000;
      var extendedSearchLogged = false;
      // 保存“点过但没打开新商品”的推荐卡 bounds，避免同一个错误区域反复点击。
      ...
      while (Date.now() < hardEndAt && (stayUntilHardEnd || detailBrowsedCount < remainingCards)) {
        ...
        // extractVisibleText() 从当前屏幕无障碍树汇总可见文本，后面所有页面判断都基于这个快照。
        var detailText = extractVisibleText();
        sample = detailText.slice(0, 220);
        // 商品卡直播组合任务会用直播入口；纯商品卡养号 skipLiveCards=true，所以这里不会进直播间。
        ...
        if ((stayUntilHardEnd || detailBrowsedCount < remainingCards) &&
          ...
          // 点击前记录页面签名；点击后如果签名没变，说明没有进入新商品详情。
          ...
          if (relatedClick && relatedClick.clicked) {
            ...
            // 如果点了但没有进入新商品，把这个 bounds 记入 skippedRelatedBounds，后面继续下滑找下一块。
        ...
        // 到达当前商品计划停留时间仍没点到后续卡时，不退出详情页，而是延长一段时间继续往下找推荐区。
        ...
        // 每 15 秒下滑一次详情页，直到找到推荐商品卡、任务暂停或总时间结束。
    ...
    // 外层先打开商城搜索结果页。失败时返回给 runner，由 runner 上报阶段失败。
    if (!openCommerceCardSearch(searchKeyword)) {
      return {
        success: false,
        reason: lastSearchFailureReason || "commerce_search_failed",
    ...
    var browsedCount = 0;
    var lastTextSample = "";
    // 外层循环处理“搜索结果页首卡”：如果详情页循环没完成目标数量，会回到搜索结果继续下滑找下一张首卡。
    while (Date.now() < endAt && (requireFullScan || browsedCount < cardCount)) {
      ...
      // 如果抖音跳回视频流或其他页面，先尝试恢复到商城搜索结果；恢复失败再结束任务。
      ...
      // 当前搜索结果页有目标关键词时，先点“全部”列表里的商品卡首卡。
      if (keywordMatched && clickCommerceKeywordCard(matchKeywords)) {
        ...
        // 进入详情后，由内部循环继续完成“推荐商品卡 -> 新详情页 -> 再推荐”的链路。
