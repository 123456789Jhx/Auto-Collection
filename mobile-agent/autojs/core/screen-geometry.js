...
function normalizeScreenSize(screenSize) {
  ...
  return {
    width: Math.max(
      1,
      isNumber(screenSize.width)
        ? Math.floor(screenSize.width)
        : DEFAULT_SCREEN_SIZE.width,
    ),
    height: Math.max(
      1,
      isNumber(screenSize.height)
        ? Math.floor(screenSize.height)
...
function coordinateFromRatio(length, ratio) {
  return clamp(
    Math.round(length * clamp(isNumber(ratio) ? ratio : 0, 0, 1)),
    0,
    length - 1,
  );
}
function clampPoint(point, screenSize) {
  ...
  return {
    x: clamp(
      point && isNumber(point.x) ? Math.round(point.x) : 0,
      0,
      size.width - 1,
    ),
    y: clamp(
      point && isNumber(point.y) ? Math.round(point.y) : 0,
      0,
      size.height - 1,
    ),
  };
...
function clampRegion(region, screenSize) {
  ...
  region = region || {};
  var left = clamp(
    Math.floor(isNumber(region.left) ? region.left : 0),
    0,
    size.width - 1,
  );
  var top = clamp(
    Math.floor(isNumber(region.top) ? region.top : 0),
    0,
    size.height - 1,
  );
  var width = Math.max(
    1,
    Math.floor(isNumber(region.width) ? region.width : 1),
  );
  var height = Math.max(
    1,
    Math.floor(isNumber(region.height) ? region.height : 1),
  );
  return {
...
function regionFromRatio(ratio, screenSize, name) {
  ...
  if (ratio.independentSize === true)
    return clampRegion(
      {
        name: name,
        left: Math.floor(size.width * leftRatio),
        top: Math.floor(size.height * topRatio),
        width: Math.ceil(size.width * ratio.width),
        height: Math.ceil(size.height * ratio.height),
      },
      size,
    );
  var rightRatio = clamp(
    leftRatio + (isNumber(ratio.width) ? ratio.width : 0),
    0,
    1,
  );
  var bottomRatio = clamp(
    topRatio + (isNumber(ratio.height) ? ratio.height : 0),
    0,
    1,
  );
  ...
  var bottom = clamp(
    Math.ceil(size.height * bottomRatio),
    top + 1,
    size.height,
  );
  return {
...
function getSwipe(direction, screenSize, options) {
  var size = normalizeScreenSize(screenSize);
  options = options || {};
  var ratios = options.ratios || SWIPE_RATIOS[direction];
  ...
  return {
    ...
    durationMs: isNumber(options.durationMs)
...
module.exports = {
  ...
  coordinateFromRatio: coordinateFromRatio,
  clampPoint: clampPoint,
  clampRegion: clampRegion,
  regionFromRatio: regionFromRatio,
  getSwipe: getSwipe,
};
